...
// Specular-AA
  ...
  uniform float m_AlphaDiscardThreshold;
    ...
    uniform sampler2D m_ShieldPattern;
    uniform float m_ShieldScale;
    uniform vec4 m_ShieldColor;
    uniform float m_ShieldIntensity;
    uniform float m_ShieldReverbSpeed;
    uniform float m_ShieldReverbAmp;
#endif
...
void main(){
    ...
    #ifdef BODY_SHIELD
        vec2 shieldTexCoord = fract(newTexCoord * m_ShieldScale);
        float shieldFactor = texture2D(m_ShieldPattern, shieldTexCoord).r;
        vec4 finalShieldColor = mix(vec4(0.0), m_ShieldColor, shieldFactor);
        float reverb = sin(g_Time * m_ShieldReverbSpeed) * m_ShieldReverbAmp;
        gl_FragColor.rgb += finalShieldColor.rgb * finalShieldColor.a * (m_ShieldIntensity + reverb);
    #endif
